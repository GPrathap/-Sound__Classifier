 ...
 */
public class AreaPolynomialApproximation extends jAudioFeatureExtractor.AudioFeatures.FeatureExtractor {
	...
	public AreaPolynomialApproximation() {
		...
		String[] attributes = new String[] { "horizontal size (window length)",
											"vertical size (number of mfcc dimensions)",
	...
	public void setElement(int index, String value) throws Exception {
		switch (index){
			...
			try {
				...
				if (val < 1) {
					throw new Exception(
						"Area Polynomial Approximation mfcc dimension length must be positive");
				} else {
					featureLength = val;
					terms = new DenseDoubleMatrix2D(k*l,windowLength*featureLength);
					z = new DenseDoubleMatrix2D(1,featureLength*windowLength);
				...
				throw new Exception(
						"vertical (mfcc dimensions) of Area Polynomial Approximation must be an integer");
			}
			break;
