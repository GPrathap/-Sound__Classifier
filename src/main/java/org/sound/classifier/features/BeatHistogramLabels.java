...
/**
 * A "mfcc extractor" that calculates the bin labels, in beats per minute, of
 * a beat histogram.
 *
...
public class BeatHistogramLabels extends FeatureExtractor {
	...
	public BeatHistogramLabels()
	{
		...
		String description = "The bin label, in beats per minute, of each beat " + 
		                     "histogram bin. Not useful as a mfcc in itself, " +
		...
		throws Exception
	{
		...
		if (beat_histogram != null)
		{
			...
			double[] labels = jAudioFeatureExtractor.jAudioTools.DSPMethods.getAutoCorrelationLabels( effective_sampling_rate,
				                                                                        min_lag,
				                                                                        max_lag );
